...
The delivered subset covers one verified local capture, one source-bound immutable analysis revision, one published profile revision with immutable leaf evidence, and immutable evidence-linked assessment revisions. The CLI and loopback API expose locked context, structure-only validation, create-only publication, history/current/detail reads and freshness. The dashboard presents the assessment snapshot, exact source quotes, requirement modality/verdict, evidence IDs/claim paths when the matching context agrees, preference checks, blockers, anomalies, questions, stale reasons and local recovery instructions.
